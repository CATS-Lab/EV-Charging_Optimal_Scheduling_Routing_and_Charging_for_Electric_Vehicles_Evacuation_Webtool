...
async function loadMap() {
	...
	for (var i = 0; i < nodeCs.length; i++) {
		...
		if (i == nodeCs.length - 1) {
			var tag = '<p>Arrive ' + arrC[i].replaceAll('"', '') + ' at ' + arrT[i] + ' with electricity level of ' + arrE[i] + '%. ' + '</p>';
		}
		else {
			tag = '<p>Leave ' + arrC[i].replaceAll('"', '') + ' at ' + arrT[i] + ' with electricity level of ' + arrE[i] + '%. ' + '</p>';
		}
